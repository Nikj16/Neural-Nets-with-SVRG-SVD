...
print_var= []
print_var1= []
f1 = open('ogsvrgf1.txt','w')
f2 = open('ogsvrgf2.txt','w')
...
def define_model(training):
    ...
    model.add(Conv2D(6, (5,5), activation='sigmoid', input_shape=(32,32,3), trainable=training))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(16, (5,5), activation='sigmoid'))
    model.add(MaxPooling2D(pool_size=(2, 2), trainable=training))
    model.add(Flatten())
    model.add(Dense(120, activation='sigmoid', trainable=training))
    model.add(Dense(84, activation='sigmoid', trainable=training))
